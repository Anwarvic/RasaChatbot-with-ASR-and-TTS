...
class MaskConv(nn.Module):
    def __init__(self, seq_module):
        ...
        sizes change during inference.
        Input needs to be in the shape of (BxCxDxT) where
            * B is the batch-size
            * C is the number of channels in the audio
            * D is 
            * T is the audio time-steps
...
class DeepSpeech(nn.Module):
    ...
    def get_model_layers(self):
        layers = list(self.conv.seq_module) + list(self.rnns) + list(list(self.fc)[0].module)
        return  layers

    def make_last_layers_trainable(self, n=0):
        layers = self.get_model_layers()
        if n == 0:
            n=len(layers)
        first_trainable_layer_idx = len(layers) - n
        for layer_idx, layer in enumerate(layers):
            requires_grad = True if layer_idx >= first_trainable_layer_idx \
                                else False
    ...
    def forward(self, x, lengths):
        ...
        #where N=B (no. of samples per batch)
        #      T=T (timesamples returned from conv)
        #      H= (width*height returned from conv)
    ...
    def get_seq_lens(self, input_length):
        ...
        return a 1D tensor or variable containing the size sequences that will
        be output by the network.
        This is applied on to Convolution layers (ANWARVIC)
